...
- **REPL namespace pre-imports**: `composition`, `subsequence`, `gm_drums`, `notes`/`midi_notes` (= `constants.midi_notes`, e.g. `notes.C3` = 48), `random`, `math`, `rich` (from `live_server.py` `_build_namespace`), plus `feeds`, `live`, `pat`, `drums` (from the `pomski_template.py` override)
